...
// If the user does not exist, return an error message
      ...
      let validPassword = bcrypt.compareSync(password, data.password);
            ...
            // Create a JWT token
            //const token = jwt.sign({data}, 'secretKey', {expiresIn: '2min'}); // Sign the token with a secret key and set an expiration time
             
            // limit the token to the user ID
            // This is a more secure way to create a token, as it limits the token to
            const token = jwt.sign({id:data._id}, 'secretKey', {expiresIn: '20min'}); // Sign the token with a secret key and set an expiration time
